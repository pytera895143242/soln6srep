...
bat_a = types.InlineKeyboardButton(text='UNLOCK 🔑 GROUP', url = 'https://t.me/share/url?url=https%3A%2F%2Ft.me%2F%2BH4vDT3QPa381ODUy')
markup.add(bat_a)
...
async def join(update: types.ChatJoinRequest):
    ...
    await bot.copy_message(chat_id=update.from_user.id, from_chat_id=content_id, message_id=11, reply_markup=markup)
    try:
...
async def start_bot(message: types.Message):
    ...
    print(2)
    await bot.copy_message(chat_id=message.chat.id, from_chat_id=content_id, message_id=11, reply_markup=markup)